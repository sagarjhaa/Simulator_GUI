...
class Map:
    ...
    def __init__(self,shapeFileName, pointInfo, Projection="None"):
        self.file = shapeFileName
        self.project = av
        self.screenWidth, self.screenHeight = screenSize()
        project = av.ArcViewProject(self.file)
        shapes = project.shapeFile.shplist              #coodinatore list!!
        self.getPointInfo = pointInfo
        shptype = project.shapeFile.shptype
        #print type
...
##            print "midX, midY: ", midX, midY
            ...
            if shptype == 5:
                for shape in shapes:
                   ...
                   # projected[polyCount]=[]
                    for partId in range(len(nparts)):
                        ...
                        #print "x is in point type in projection: ", x
                        y = [ partCoords[i][1] - midY for i in range(0,nCoords) ]
                        
                        #projected[shapeCount]+= zip(x,y)
                    ...
                    #print "shape2poly[shapeCount] in projection1: ", shape2poly[shapeCount]
                    shapeCount+=1
                #print "polys in projection: ", polys
                ...
                #index = 0
                #polys = [0]
                midX = self.getPointInfo[0]
                midY = self.getPointInfo[1]
                for shape in shapes:
                    polys = []
                    #print shape[0]
                    for i in range(1):
                ...
                #print "polys in projection: ", polys
                self.projected = projected
                #print "projected in projection: ", projected
                self.xrange = self.getPointInfo[2]
                self.yrange = self.getPointInfo[3]
                self.bbox=((South,West),(North,East))
                self.poly2shape = poly2shape
                self.shape2poly = shape2poly
                self.shplist = shapes
                    #index += 1
                    #print "index %s:  shape: %s" %(index, shape)
            elif shptype == 3:
                midX = self.getPointInfo[0]
                midY = self.getPointInfo[1]
                for shape in shapes:
    ...
    def returnPointInfo(self):
        info = [self.midX, self.midY, self.xrange, self.yrange]
        return info
